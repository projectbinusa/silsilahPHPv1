	...
	if(!empty($searsh)) {
	 	if($sql->num_rows){
		 	while($rs=$sql->fetch_assoc()){
			 	$aa .= "<li><a href='".path."/tree.php?id={$rs['id']}&t=".fh_seoURL($rs['name'])."'>{$rs['name']}</a></li>";
		 	}
		} else {
...
elseif($pg == 'search1'){
	$searsh1 = sc_sec($_POST["search1"]);
	$aa = '';
	$sql = $db->query("SELECT * FROM ".prefix."members WHERE firstname LIKE '%{$searsh1}%' LIMIT 14");
	$aa .='<ul class="pt-drop">';
   if(!empty($searsh1)) {
		if($sql->num_rows){
			while($rs=$sql->fetch_assoc()){
...
elseif($pg == 'send-newmember'){
	...
	if(isset($_POST['images_tmp'])){
		echo print_r($_POST['images_tmp'],truﬁe);
